...
void *Talk(void *talkSocket)
{
    ...
    char buffer[2048];
    int nbytes = recv(fd, buffer, sizeof(buffer), NULL);
    if (nbytes == -1)
    {
        std::cerr << "[Talk] reading file failed\n";
        return;
    }
...
int main(int argc, char **argv)
{
    ...
    if (errno == -1)
    {
        std::cerr << "error creating listenSocket\n";
        return -1;
    }
    ...
    int bindListenSocket = bind(listenSocket, (struct sockaddr *)&listenAddress,
                                sizeof(struct sockaddr));
    if (errno == -1)
    {
        std::cerr << "error binding listenSocket\n";
        return -1;
    }
    ...
    while ((talkAddressLength = sizeof(talkAddress),
            talkSocket = accept(listenSocket, (struct sockaddr *)&talkAddress,
                                &talkAddressLength)) &&
           talkSocket != -1)
    {
